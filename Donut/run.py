import warnings
import json
import os
from PIL import Image
from datasets import Dataset, load_from_disk
from transformers import (
    DonutProcessor,
    VisionEncoderDecoderModel,
    Seq2SeqTrainer,
    Seq2SeqTrainingArguments,
    default_data_collator
)
import torch
import urllib.request

# === DOWNLOAD DATA FROM DROPBOX ===
print("=== Downloading and extracting dataset from Dropbox ===\n")
zip_url = "https://www.dropbox.com/scl/fi/sviiv9yv6c05yxwrnwvoz/data.zip?rlkey=068sh3m0f1zqvnuksdfpu56qf&st=0dw5agtd&dl=1"
if not os.path.exists("data.zip"):
    urllib.request.urlretrieve(zip_url, "data.zip")

os.system("unzip -o data.zip -d ./")

# === CONFIG ===
ANNOTATIONS_PATH = "annotations.json"
MODEL_NAME = "naver-clova-ix/donut-base"
OUTPUT_DIR = "./donut_sensitive_model"
LOG_DIR = "./logs"
MAX_LENGTH = 512
USE_SAVED = False  # ✅ set to True if you’ve saved processed datasets to disk

# === Load Donut Processor and Model ===
print("=== Loading Donut model and processor === \n")
processor = DonutProcessor.from_pretrained(MODEL_NAME, use_fast=True)
model = VisionEncoderDecoderModel.from_pretrained(MODEL_NAME)

model.config.decoder_start_token_id = processor.tokenizer.convert_tokens_to_ids(["<s>"])[0]
model.config.pad_token_id = processor.tokenizer.pad_token_id
model.config.loss_type = "ForCausalLMLoss"

# === Load and Preprocess Dataset ===
if not USE_SAVED:
    print("=== Loading annotations === \n")
    with open(ANNOTATIONS_PATH, "r") as f:
        raw_data = json.load(f)

    # Limit for testing (adjust as needed)
    train_data = raw_data[:50]
    val_data = raw_data[50:60]

    # train_data = raw_data[:int(0.9 * len(raw_data))]
    # val_data = raw_data[int(0.9 * len(raw_data)):]

    print(f"Training examples: {len(train_data)} | Validation examples: {len(val_data)} \n")

    def preprocess(example):
        # Fix Windows-style backslashes
        image_path = example["image_path"].replace("\\", "/")
        try:
            with Image.open(image_path).convert("RGB") as image:
                pixel_values = processor(image, return_tensors="pt").pixel_values.squeeze(0)
        except Exception as e:
            print(f"Error opening image {image_path}: {e}")
            pixel_values = torch.zeros((3, 256, 256))  # dummy tensor

        target_text = example["task_prompt"] + json.dumps(example["ground_truth"], separators=(",", ":"))
        input_ids = processor.tokenizer(
            target_text,
            padding="max_length",
            max_length=MAX_LENGTH,
            truncation=True,
            return_tensors="pt"
        ).input_ids.squeeze(0)
        return {
            "pixel_values": pixel_values,
            "labels": input_ids
        }

    print("=== Processing dataset === \n")
    train_ds = Dataset.from_list(train_data).map(preprocess, batched=False, writer_batch_size=5)
    val_ds = Dataset.from_list(val_data).map(preprocess, batched=False, writer_batch_size=5)

    # Optional: Save to disk
    train_ds.save_to_disk("train_dataset")
    val_ds.save_to_disk("val_dataset")
else:
    print("=== Loading preprocessed dataset from disk ===")
    train_ds = load_from_disk("train_dataset")
    val_ds = load_from_disk("val_dataset")

# === Training Arguments ===
training_args = Seq2SeqTrainingArguments(
    output_dir=OUTPUT_DIR,
    per_device_train_batch_size=2,
    per_device_eval_batch_size=2,
    eval_strategy="steps",
    logging_dir=LOG_DIR,
    logging_steps=10,
    save_steps=100,
    eval_steps=100,
    save_total_limit=2,
    num_train_epochs=10,
    predict_with_generate=True,
    fp16=torch.cuda.is_available(),  # will use fp16 only if GPU is available
)

# === Initialize Trainer ===
print("=== Initializing trainer ===\n")
trainer = Seq2SeqTrainer(
    model=model,
    args=training_args,
    train_dataset=train_ds,
    eval_dataset=val_ds,
    tokenizer=processor,
    data_collator=default_data_collator,
)

warnings.filterwarnings("ignore", message="Passing a tuple of `past_key_values` is deprecated")

# === Start Training ===
print("=== Starting training === \n")
trainer.train()
print("✅ Training complete.")
